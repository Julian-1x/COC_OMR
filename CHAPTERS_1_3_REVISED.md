...
**Status:** Revised content complete. Format-matched Word document: `COC_OMR_Chapters_1_4_REVISED.docx`  
**Format rules:** `paper_format_extract/FORMAT_SPEC.md` (measured from original Ch1–3 PDF)  
**Do not invent evaluation numbers, respondents, or citations.**  
**Original title is retained** so the approved title page can stay unchanged.

---

## What changed (review this first)

| Area | Original paper | Revised wording |
|------|----------------|-----------------|
| Web users | Instructors **and students** use a results portal | Teachers and authorized school administrators use the web portal. Students do **not** log in. |
| Student benefit | Students access detailed analysis in the system | Students benefit from faster checking and teacher-released feedback (exports/printouts). |
| Offline model | Works without internet | Exam-day scanning works offline **after** one online sign-in, approval, and PIN setup. |
| Answer sheets | Standard layouts only (implied) | Standard 30–100 layouts **and** scan-safe custom full-page portrait sheets. |
| Integrity features | Barely mentioned | Review-before-save, shared vs section-only keys, Exam Day Board. |
| Stack | Flutter 0.72, react-native-opencv, HTML/JS, PHP 10, MySQL | Flutter + Dart 3.x, native OpenCV, Next.js/React, Laravel 11 / PHP 8.2+, PostgreSQL. |
| Security | No unencrypted local student data; grade encryption | Local roster/scores exist for offline use; PIN hashed; photos stay on device; HTTPS/TLS for sync. |
| Evaluation respondents | 30–40 students evaluate the web portal | Evaluation focuses on **instructors** (and admins if they use Access Control). Student portal SUS is removed. |
| Literature accuracy | 99.76% cited near product claims | Kept as **prior study** result only, not COC OMR accuracy. |
| “AI OMR” | Implied product uses AI | Literature may discuss AI; this system uses OpenCV computer vision. |

Items that **need your confirmation before final typing**:

- Whether permission letters, informed consent, and Engineering interviews were actually completed.
- Final instructor respondent count (keep 5–7 only if that is your real plan or completed sample).
- ISO/IEC 25010 and SUS results belong in Chapter 4 only after you compute them.
...
In the Philippine context, several studies have identified technological and infrastructural challenges that affect digital learning tools. Borbon, Jr. et al. (2024) discussed a “Third-Level Digital Divide” in regions such as Northern and Southern Mindanao, where slow and costly internet access limits the effectiveness of digital educational tools. This supports the need for classroom systems that do not require a continuous connection during the most time-critical task—in this study, exam-day scanning and scoring.
...
Recent Philippine research provides empirical support for mobile OMR adoption. A 2024 study by Cuerdo and Sinfuego at the University of Cabuyao evaluated EvalBee, an Alternative Optical Mark Recognition (AOMR) application, using a mixed-methods approach with 200 test papers and five faculty participants. Their quantitative findings showed large efficiency gains for that application: AOMR processed test papers in 0.05 minutes compared with 6.57 minutes manually—a 130-fold increase in speed—while maintaining equivalent accuracy and reliability (Cohen’s *d* = 1.00, large effect size).
...
The synthesis also shows gaps. Cuerdo and Sinfuego (2024) demonstrated EvalBee’s efficiency but also its dependence on precise capture conditions and, in free use, possible interruptions. Pushpa et al. (2022) showed a robust OpenCV pipeline but remained offline-only. CheckIt (Patel et al., 2015) showed mobile feasibility but did not provide an institution-owned hybrid of exam-day offline grading and later teacher web analysis. These studies justify a **teacher-owned**, ad-free, offline-first scanner with later synchronization. They do not justify claiming a student results portal, nor do they transfer their accuracy percentages to this product.
...
1. **Infrastructure–technology mismatch.** Free tools such as EvalBee, as discussed by Cuerdo and Sinfuego (2024), may require internet access or expose users to advertisements. Offline-only systems such as Pushpa et al. (2022) limit later data access. Instructors at PHINMA Cagayan de Oro College need exam-day scanning that continues without Wi-Fi, plus later sync when a connection returns.

2. **Teacher-owned analytics, not a student portal.** Existing studies often emphasize scanning accuracy (Küçükkara & Tümer, 2018; Patel et al., 2015) but do not provide a localized, ad-free portal where **instructors and authorized administrators** can review synchronized results and item analysis after exam day. The gap addressed by this study is teacher-facing analysis and record continuity—not student self-service login.
...
- **Instructors** from the Department of Engineering who handle multiple-choice assessments and will use the mobile application and the teacher web portal. Approximately 5–7 instructors are the planned evaluation group. *[Retain this range only if it matches your actual sample.]*
...
1. **Permission and coordination.** Formal permission was requested from the Department of Engineering to study current examination workflows. *[Keep this sentence only if the letter was actually secured.]*
...
1. **ISO/IEC 25010 questionnaire** — to evaluate functional suitability, reliability (including offline use), performance efficiency, usability, security, and portability as perceived by instructor-users.
2. **System Usability Scale (SUS)** — to measure how easy instructors find the mobile scanner, review flow, and teacher web portal.
...
The earlier paper listed “absence of a student result portal” as a gathered requirement. The implemented system does **not** include student login. Result release remains an instructor responsibility through exports, printouts, or class discussion. That change is treated as a scope decision, not as an unimplemented defect hidden from the reader.
...
9. Generate printable standard sheets (30–100 items) and scan-safe custom full-page portrait sheets.
...
- **Teacher (Instructor)** — prepares rosters and keys, prints sheets, scans, reviews, stores, syncs, and views analysis.
- **School Administrator** — approves teacher access and may view school-level records appropriate to that role.
...
| Development tools | Visual Studio Code / Cursor, Git, Flutter SDK | — |
...
1. **Informed consent.** Participants who evaluate the system should receive an informed-consent form describing purpose, procedures, risks, and the right to withdraw. *[Retain as fact only if forms were used.]*
2. **Data privacy and confidentiality.** Synchronization uses authenticated HTTPS/TLS. The offline PIN is stored as a hash. Scan photographs remain on the device and are not uploaded. Roster names, answers, and scores are stored locally to support offline grading and are synchronized to the school server when the teacher syncs. Access is limited to the teacher’s own records and to authorized administrators. Evaluation reports should present grouped or anonymized findings rather than publishing identifiable student scores.
3. **Institutional approval.** Formal permission from the college or department should be on file before classroom data collection. *[Retain as fact only if secured.]*
...
*(References below are those already used in the original manuscript. No new sources were added.)*

Borbon, Jr., et al. (2024). Bridging the third-level digital divide: An examination of digital inequalities among different groups in higher education. *Journal of Innovative Research in Social and Educational Administration, 22*(3), 45–62. http://www.seaairweb.info/journal/articles/JIRSEA_v22_n03/JIRSEA_v22_n03_Article07.pdf

Cuerdo, R. P., & Sinfuego, et al. (2024). Enhancing face-to-face evaluation using alternative optical mark recognition: A case study from the University of Cabuyao's college of education. *HCMCOUJS-Social Sciences, 14*(1), 118–132. https://doi.org/10.46223/HCMCOUJS.soci.en.14.1.2905.2024

Küçükkara, S., & Tümer, et al. (2018). An image processing oriented optical mark recognition and evaluation system. *Journal of Engineering Technology, 6*(3), 112–125. https://www.researchgate.net/publication/330977246

Patel, R., Sanghavi, S., et al. (2015). CheckIt – A low-cost mobile OMR system. *International Journal of Computer Applications, 120*(15), 12–16. https://www.researchgate.net/publication/280776405

Pushpa, G., Mallesh, A., & Das, et al. (2022). Enhancing e-assessment with image processing for exam evaluation. *Industrial Engineering Journal, 51*(10), 100–106. https://apgcu.edu.in/pdf/mca-publications/2022-13.pdf

Silao, R. P., Luciano, et al. (2021). Effectiveness of automation in evaluating test results using EvalBee as an alternative Optical Mark Recognition (OMR): A quantitative-evaluative approach from a Philippine public school. *International Journal of Theory and Application in Elementary and Secondary School Education, 3*(2), 61–75. https://doi.org/10.31098/ijtaese.v3i2.661

---

## Notes for appendices (not rewritten in full here)

When you update the appendices, align them with this draft:

| Appendix | Required change |
|----------|-----------------|
| A / L Mockups | Remove or label student-dashboard screens as discarded early concepts. Replace with actual teacher mobile and web screens. |
| D Use Case | Actors: Teacher, School Administrator only. |
| E / P ERD and physical DB | PostgreSQL/SQLite entities; no student-user table for portal login. |
| F / G DFD / Context | Web output goes to teachers/admins. |
| I / J Proposed flow | Include PIN, review, shared/section-only keys, sync later. |
| O FR matrix | Remove student-login / student-portal FRs (old FR-19 and similar). |

Do not paste booking, scheduling, or field-worker content from the old Chapter 4 PDF into any appendix.
