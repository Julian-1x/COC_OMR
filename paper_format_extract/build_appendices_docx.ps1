# Build Appendices DOCX matching THESISREAL / FORMAT_SPEC.md spacing.
# Structure source: D:\DOWNLOADS\APPENDICES.docx.pdf (adapted for COC OMR)
# Format source: paper_format_extract/FORMAT_SPEC.md

$ErrorActionPreference = 'Stop'
$outPath = 'd:\omr_app\APPENDICES_THESISREAL_FORMAT.docx'
$md = [System.IO.File]::ReadAllText(
  'd:\omr_app\paper_format_extract\appendices_camera_ready.md',
...
while ($i -lt $lines.Count) {
  ...
  if ($line -match '^# APPENDICES\s*$') {
    if ($chapterStarted) {
      $word.Selection.InsertBreak(7) | Out-Null
    }
    $chapterStarted = $true
    Write-Styled 'APPENDICES' 'CHAPTER'
    $i++; continue
  }
  ...
  if ($line -match '^##\s+(.+)$') {
    ...
    if ($title -cmatch '^(THE PROBLEM|REVIEW OF RELATED LITERATURE AND STUDIES|METHODOLOGY|PRESENTATION, ANALYSIS, AND INTERPRETATION OF DATA|SUMMARY, CONCLUSIONS, AND RECOMMENDATIONS)$' -or ($title -eq $title.ToUpper() -and $title.Length -gt 8 -and $title -notmatch '^APPENDIX')) {
      Write-Styled $title.ToUpper() 'CHAPTER_TITLE'
    } elseif ($title -match '^REFERENCES$') {
      $word.Selection.InsertBreak(7) | Out-Null
      Write-Styled 'REFERENCES' 'CHAPTER_TITLE'
    } elseif ($title -match '^APPENDIX\s+[A-Z]') {
      # Start each major appendix on a new page for binder readability
      if ($chapterStarted) {
        $word.Selection.InsertBreak(7) | Out-Null
      }
      $chapterStarted = $true
