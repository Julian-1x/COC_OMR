...
- **Format skeleton:** `Chapt4.pdf` (section order, table numbering style, and academic tone)
- **Testing tables:** `TESTING-PLAN.pdf` as mapped in `docs/COC_OMR_Testing_Registry.html`
- **System and paper context:** `THESISREAL.pdf` and the implemented COC OMR stack
- **Automated unit totals:** Flutter `flutter test` (242 passed) + web Vitest (52 passed), recorded 21 September 2026

**Honesty rule.** ISO/IEC 25010 means, SUS scores, timed “% faster” claims, and literature accuracy percentages (for example 99.76%) are **not invented**. Where instructor forms or timed trials are not yet encoded, the subsection states that the instrument is pending.
...
### 4.2.2 Teacher / Administrator Web Portal Features
...
where \(\bar{X}\) represents the mean score, \(\sum X\) is the sum of all individual scores, and \(N\) represents the total number of evaluators.

**This draft does not invent \(N\), mean scores, or SUS composites.** Insert computed values only after forms are encoded.
...
| Quality characteristic | Mean score (\(\bar{X}\)) | Qualitative interpretation |
|------------------------|-------------------------:|----------------------------|
...
| Integration testing (IT-101–IT-104 + path checks) | 4 (+ path checks) | 4 | 0 | 100% |
